...

{
    ...
    public function driverListWithAgent($search)

    {
        if($search != "" && $search != Null) {
            $this->db->where('agent_id', $search);
        }

        $this->db->select('config_driver.status as status_job, customer.customer_fullname');
        ...
        $this->db->join('config_driver', 'driver.id = config_driver.driver_id', 'left');
        $this->db->join('customer', 'driver.agent_id = customer.id', 'left');
        //$this->db->join('driver', 'customer.id = driver.agent_id', 'left');
