...
public class AgentOrders extends AppCompatActivity  {
    ...
    public void initLocationEngine() {
        if (PermissionsManager.areLocationPermissionsGranted(AgentOrders.this)) {
            locationEngine = LocationEngineProvider.getBestLocationEngine(AgentOrders.this);
            ...
            driverService.freshorder(param).enqueue(new Callback<AllTransResponseJson>() {
                ...
                public void onResponse(@NonNull Call<AllTransResponseJson> call, @NonNull Response<AllTransResponseJson> response) {
                    if (response.isSuccessful()) {
                        shimmertutup();
                        historyItem = new HistoryItem(AgentOrders.this, Objects.requireNonNull(response.body()).getData(), R.layout.incoming_item_order);
