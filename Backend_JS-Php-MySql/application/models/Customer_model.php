...
class Customer_model extends CI_model
{
    ...
    {
        $this->db->select('transaction.*,merchant_detail_transaction.*,service.*,merchant.*,transaction_history.*,detail_send_transaction.*,
            customer.latitude as agent2_latitude,
            customer.longitude as agent2_longitude,
            customer.address as agent2_address,
            customer.city as agent2_city,
            customer.state as agent2_state,
            customer.zipcode as agent2_zipcode,
            customer.phone_number as agent2_phone_number,
            customer.customer_fullname as agent2_customer_fullname,
            customer1.latitude as agent1_latitude,
            customer1.longitude as agent1_longitude,
            customer1.address as agent1_address,
            customer1.city as agent1_city,
            customer1.state as agent1_state,
            customer1.zipcode as agent1_zipcode,
            customer1.phone_number as agent1_phone_number,
            customer1.customer_fullname as agent1_customer_fullname');
        $this->db->from('transaction');

        $this->db->join('merchant_detail_transaction', 'transaction.id = merchant_detail_transaction.transaction_id', 'left');
        $this->db->join('customer', 'customer.id = transaction.destination_customer_id', 'left');
        $this->db->join('customer as customer1', 'customer1.id = transaction.customer_id', 'left');
        $this->db->join('service', 'transaction.service_order = service.service_id', 'left');
        $this->db->join('merchant', 'merchant_detail_transaction.merchant_id = merchant.merchant_id', 'left');
...
<!--[if (mso)|(IE)]></td></tr></table><![endif]-->
    ...
    public function insert_data_transaksi_merchant($data_transaksi, $dataDetail, $total_belanja)
    {
        ...
        $transaction = [
            ...
            'destination_customer_id' => $data_transaksi['destination_customer_id'],
    ...
    /*public function get_customer_by_distance($dec_data){
            $this->db->select("id,6371 * 2 * ASIN(SQRT(POWER(SIN(RADIANS($dec_data->end_latitude - ABS(latitude))), 2) + COS(RADIANS($dec_data->end_latitude)) * COS(RADIANS(ABS(latitude))) * POWER(SIN(RADIANS($dec_data->end_longitude - longitude)), 2))) AS distance,
            ...
            $this->db->from('customer');
            $this->db->where('id !=', $dec_data->customer_id);
    ...
    public function get_customer_by_distance($dec_data){
            ...
            $this->db->from('customer');
            $this->db->where('id !=', $dec_data->customer_id);
            $this->db->where('zipcode', $dec_data->zipcode);
            //$this->db->having('distance < 50');
            $this->db->order_by('distance','ASC');
       return $this->db->get()->result();
    }

    function all_tran_receive_order_history($iduser)
    {
        $this->db->select('*');
        $this->db->from('transaction');
        $this->db->join('transaction_history', 'transaction.id = transaction_history.transaction_id', 'left');
        $this->db->join('transaction_status', 'transaction_history.status = transaction_status.id', 'left');
        $this->db->join('service', 'transaction.service_order = service.service_id', 'left');
        $this->db->where('transaction.destination_customer_id', $iduser);
        //$this->db->where('transaction_history.status != 1');
        $this->db->where('transaction_history.status != 0');
        $this->db->order_by('transaction.id', 'DESC');
        $trans = $this->db->get();
