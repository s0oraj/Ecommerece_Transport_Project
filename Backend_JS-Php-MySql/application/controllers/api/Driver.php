...

{
...

    {
        ...
        $receive_transaksi = $this->Driver_model->receive_request($data_req);

        if ($receive_transaksi['status']) {
    ...
    function accepted_orders_list_post()
    {
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.0 401 Unauthorized");
        ...
        // $orders = $this->Driver_model->accepted_order($data_req);
        $orders = $this->Driver_model->accepted_orderAll($data_req);
    ...
    function receiver_orders_list_post()
    {
    ...
    if (!isset($_SERVER['PHP_AUTH_USER'])) {
      ...
      $orders = $this->Driver_model->receiverOrderAll($data_req);
      ...
      $this->response(array( 'status' => true, 'data' => array()), 500);
    }
