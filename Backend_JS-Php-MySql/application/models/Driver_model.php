...

{
...

    {
        ...
        public function receive_request($cond)

    {
          ...
          // $this->db->where($cond);
          $this->db->where(array('transaction_id' => $cond['transaction_id']));
    ...
    public function order_filter_by_geo($Driver)

    {
        ...
        $result = $this->db->query("select transaction.*,transaction_history.*,merchant_detail_transaction.*,transaction_status.*,service.*,transaction.id as transaction_id,

            

            6371 * 2 * ASIN(SQRT(POWER(SIN(RADIANS($lat - ABS(transaction.start_latitude))), 2) + COS(RADIANS($lat)) * COS(RADIANS(ABS(transaction.start_latitude))) * POWER(SIN(RADIANS($long - transaction.start_longitude)), 2))) AS distance

            FROM transaction

            LEFT JOIN  merchant_detail_transaction ON transaction.id = merchant_detail_transaction.transaction_id

            LEFT JOIN  transaction_history ON transaction.id = transaction_history.transaction_id

            LEFT JOIN  transaction_status ON transaction_history.status = transaction_status.id
    ...
    public function order_filter_by_geo_new($Driver)
    {
        ...
        /*$result = $this->db->query("select transaction.*,transaction_history.*,merchant_detail_transaction.*,transaction_status.*,service.*,transaction.id as transaction_id,
            ...
            LEFT JOIN  service ON transaction.service_order = service.service_id
            WHERE transaction_history.status = 1
            WHERE transaction.customer_id = $agent_id
            HAVING distance < 20
            ORDER BY transaction.id DESC
            ");*/

        $this->db->select('transaction.*,transaction_history.*,merchant_detail_transaction.*,transaction_status.*,service.*,transaction.id as transaction_id');
        ...
        $this->db->where('transaction.customer_id', $agent_id);
        $this->db->where('transaction.driver_id', Null);
        //$this->db->where('order_time BETWEEN DATE_SUB(NOW(), INTERVAL 10 DAY) AND NOW()');
        //$this->db->get();
    ...
    public function agent_list_near_by_driver($Driver)
    {
        $lat = $Driver['lat'];
        $long = $Driver['long'];
        $this->db->select("id,6371 * 2 * ASIN(SQRT(POWER(SIN(RADIANS($lat - ABS(latitude))), 2) + COS(RADIANS($lat)) * COS(RADIANS(ABS(latitude))) * POWER(SIN(RADIANS($long - longitude)), 2))) AS distance,
                latitude as agent2_latitude,
                longitude as agent2_longitude,
                address as agent2_address,
                city as agent2_city,
                state as agent2_state,
                zipcode as agent2_zipcode,
                phone_number as agent2_phone_number,
                customer_fullname as agent2_customer_fullname");
            $this->db->from('customer');
    ...
    public function agents_order_count($agent_id)
    {
        //$this->db->where($this->db->where('driver_id', Null);
        $this->db->where('driver_id', Null);
        //$this->db->where($this->db->where('order_time BETWEEN CURDATE() - INTERVAL 10 DAY AND CURDATE()'));
        //$this->db->where('order_time BETWEEN DATE_SUB(NOW(), INTERVAL 10 DAY) AND NOW()');
        $query = $this->db->get_where('transaction', ['customer_id' => $agent_id])->result_array();
        //print_r($this->db->last_query());die();
        return count($query);
    }

    public function accepted_order($Driver)
    {
        ...
        $this->db->where('transaction_history.status', 2);
        $this->db->where('transaction.driver_id', $driver_id);
        $this->db->group_by('transaction.customer_id');
        return  $this->db->get('transaction')->result_array();
        //print_r($this->db->last_query());die();

    }
    
    public function accepted_orderAll($Driver)
    {
        $driver_id = $Driver['driver_id'];
        ...
        $this->db->where('transaction.driver_id', $driver_id);
        $where = '(transaction_history.status = "2" or transaction_history.status = "3")';
        $this->db->where($where);
        return  $this->db->get('transaction')->result_array();
        //print_r($this->db->last_query());die();
    }
    
    public function receiverOrderAll($Driver){
        $agent_id = $Driver['agent_id'];
        $this->db->select('service.service, transaction.id as transaction_id,transaction_history.status');
      
        $this->db->join('transaction_history', 'transaction.id = transaction_history.transaction_id', 'left');
        $this->db->join('service', 'transaction.service_order = service.service_id', 'left');
        $this->db->where('transaction.destination_customer_id', $agent_id);
        $this->db->where('transaction_history.status != "6"');
        $res = $this->db->get('transaction')->result_array();
        return $res;
    }
}
