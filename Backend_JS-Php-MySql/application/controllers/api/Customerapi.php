...
class Customerapi extends REST_Controller
{
        ...
        if ($check_exist) {
            ...
            if ($dec_data->checked == "true") {
                ...
                $data_signup = array(
                    ...
                    'city' => $dec_data->city,
                    'state' => $dec_data->state,
                    'zipcode' => $dec_data->zipcode,
                );
    ...
    function request_transaksi_post()
    {
        ...
        $data_req = array(
            'customer_id' => $dec_data->customer_id,
            //'destination_customer_id' => $dec_data->customer_id,
    ...
    function detail_transaksi_post()
    {
        ...
        if(property_exists($dec_data,'driver_id')){
             $getdriver = $this->Customer_model->detail_driver($dec_data->driver_id);
             $getdriver = $getdriver->result();
         }else{
             $getdriver = array();
         }
    ...
    function request_agents_list_post()
    {
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.0 401 Unauthorized");
        ...
        $result = $this->Customer_model->get_customer_by_distance($dec_data);
        if(empty($result)){
            $message = array(
                'message' => 'No Agent found around distination area',
                'data' => []
            );
            $this->response($message, 200);
        }
    ...
    function request_transaksi_send_post()
    {
        ...
        $data = file_get_contents("php://input");
        $dec_data = json_decode(trim(file_get_contents('php://input')), true);
        //$dec_data = json_decode($data);
        //$this->response($dec_data, 200);

        /*$get_customer_by_geo = $this->Customer_model->get_customer_by_distance($dec_data);
        //echo $this->db->last_query();exit;
        $destination_customer_id = "";
        if(empty($get_customer_by_geo)){
            $message = array(
                'message' => 'No Agent found around distination area',
                'data' => []
            );
        ...
        //print_r($get_customer_by_geo);exit;
        ...
        // $path = "images/parcel/" . $namafoto;

        // file_put_contents($path, base64_decode($image));
        $data_new = file_get_contents("php://input");
        $dec_data_new = json_decode($data_new);
        $imagesim = $dec_data_new->photo;
        ...
        $data_req = array(
            'customer_id' => $dec_data['customer_id'],
            //'destination_customer_id' => $get_customer_by_geo[0]->id,
            'destination_customer_id' => $dec_data['destination_customer_id'],
            'service_order' => $dec_data['service_order'],
            'start_latitude' => $dec_data['start_latitude'],
            'start_longitude' => $dec_data['start_longitude'],
            'end_latitude' => $dec_data['end_latitude'],
    ...
    function receive_order_history_post()
    {
        if (!isset($_SERVER['PHP_AUTH_USER'])) {
            header("WWW-Authenticate: Basic realm=\"Private Area\"");
            header("HTTP/1.0 401 Unauthorized");
        ...
        $data = file_get_contents("php://input");
        $decoded_data = json_decode($data);
        $getWallet = $this->Customer_model->all_tran_receive_order_history($decoded_data->id);
        $message = array(
            'status' => true,
            'data' => $getWallet->result()
        );
